...
const EditBox: Component<{
  ...
  const [articleRefs, setArticleRefs] = createStore<
    Record<string, PrimalArticle>
  >({});
  const [highlightRefs, setHighlightRefs] = createStore<Record<string, any>>(
    {}
  );
  const [liveEventRefs, setLiveEventRefs] = createStore<
    Record<string, StreamingData>
  >({});
  ...
  const [referencedNotes, setReferencedNotes] =
    createStore<Record<string, FeedPage>>();
  const [referencedArticles, setReferencedArticles] =
    createStore<Record<string, FeedPage>>();
  ...
  const onExpandableTextareaInput: (event: InputEvent) => void = (event) => {
    ...
    if (
      ...
      elm.id !== `${prefix()}new_note_text_area` ||
      !preview
    ) {
      return;
    }
  ...
  const renderMessage = () => {
    ...
    if (!noteHasInvoice(text)) {
      return (
        <div class={styles.editor} ref={textPreview} innerHTML={text}></div>
      );
    }
    ...
    let content = text
      .replace(linebreakRegex, " __LB__ ")
      .replaceAll("<br>", " __LB__ ")
      .replace(/\s+/g, " __SP__ ");
    ...
    return (
      <div class={styles.editor} ref={textPreview}>
        <For each={sections}>
          {(section) => (
            <Switch fallback={<div innerHTML={section}></div>}>
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (!isMentioning() && !isEmojiInput() && e.key === ":") {
      ...
      emojiCursorPosition = getCaretCoordinates(
        textArea,
    ...
    if (isEmojiInput()) {
      if (e.code === "ArrowDown") {
        ...
        const emojiHolder = document.getElementById(
          `${instanceId}-${highlightedEmoji()}`
        );

        if (
          emojiHolder &&
          emojiOptions &&
          !isVisibleInContainer(emojiHolder, emojiOptions)
        ) {
    ...
    if (!isMentioning() && e.key === "@") {
      mentionCursorPosition = getCaretCoordinates(
        textArea,
        textArea.selectionStart
      );
    ...
    if (!isMentioning() && e.code === "Backspace" && textArea) {
      ...
      if (lastWord?.startsWith("@`")) {
        ...
        const newText =
          textSoFar.slice(0, index) + textArea.value.slice(cursor);
    ...
    if (isMentioning()) {
      ...
      const cursor = textArea.selectionStart;
      const lastMentionTrigger = textArea.value
        .slice(0, cursor)
        .lastIndexOf("@");
  ...
  createEffect(() => {
    if (props.open) {
      const draft = readNoteDraft(
        account?.publicKey,
        props.replyToNote?.noteId
      );
      const draftUserRefs = readNoteDraftUserRefs(
        account?.publicKey,
        props.replyToNote?.noteId
      );
  ...
  const postNote = async () => {
    ...
    if (account) {
      ...
      if (rep && rep.naddr) {
        ...
        const rHints =
          rep.relayHints && rep.relayHints[rep.id]
      ...
      if (rep && !rep.naddr) {
        ...
        // Otherwise, add the note as the root tag for this reply
          ...
          tags.push(["e", rep.id, rHints, "root"]);
        }
        ...
        const unsub = subsTo(importId, {
          onEose: () => {
            if (note) {
              ...
              props.onSuccess &&
                props.onSuccess(
                  { success, reasons, note },
                  { noteRefs, userRefs, articleRefs, highlightRefs, relayHints }
                );
  ...
  const postSubscriberNote = async () => {
    if (!account || !account.hasPublicKey() || fileToUpload()) {
      return;
    }
    ...
    // Fetch user relay info for mentions
    let userRelays = await new Promise<Record<string, string[]>>((resolve) => {
      const uids = Object.values(userRefs).map((u) => u.pubkey);
      const subId = `users_relays_${APP_ID}`;
      let relays: Record<string, string[]> = {};
    ...
    // Parse mentions to nprofile format
    const messageToSend = value.replace(editMentionRegex, (url) => {
      const atIndex = url.indexOf("@");
      const anythingBefore = url.slice(0, atIndex);
      const mention = url.slice(atIndex);
    ...
    if (account) {
      ...
      // --- SUBSCRIBER ONLY ACCESS CONTROL TAGS ---
      tags.push(["subscribers_only", "true"]);
      // tags.push(["p", account.publicKey]); // indicate creator if you want
      tags.push(["p", "account.publicKey"]); // indicate creator if you want
  ...
  const highlightHashtags = (text: string) => {
    ...
    return text.replace(regex, (token) => {
      ...
      const embeded = (
        <span>
          {space}
          <span class={styles.userReference}>#{term}</span>
        </span>
      );
  ...
  const parseUserMentions = (text: string) => {
    return text.replace(editMentionRegex, (url) => {
      ...
      const user = Object.values(userRefs).find(
        ...
        const link = user ? (
          <a
            href={`${window.location.origin}${
              app?.actions.profileLink(user.npub) || ""
            }`}
            target="_blank"
            class="linkish"
          >
      ...
      const unsub = subsTo(subId, {
        onEvent: (_, content) => {
          ...
          if (content.kind === Kind.Metadata) {
            ...
            setReferencedArticles(id, "users", (usrs) => ({
              ...usrs,
              [user.pubkey]: { ...user },
            }));
            return;
          ...
          if (
            [Kind.LongForm, Kind.LongFormShell, Kind.LiveEvent].includes(
              content.kind
            )
          ) {
            const message = content as NostrNoteContent;

            setReferencedArticles(id, "messages", (msgs) => [
              ...msgs,
              { ...message },
            ]);
          ...
          if (content.kind === Kind.NoteStats) {
            ...
            setReferencedArticles(id, "postStats", (stats) => ({
              ...stats,
              [stat.event_id]: { ...stat },
            }));
            return;
          ...
          if (content.kind === Kind.Mentions) {
            ...
            setReferencedArticles(id, "mentions", (mentions) => ({
              ...mentions,
              [mention.id]: { ...mention },
            }));
            return;
  ...
  const subNaddrRef = async (noteId: string) => {
    const parsed = await replaceAsync(
      parsedMessage(),
      eventRegexG,
      async (url) => {
        let id = url;
      ...
      const unsub = subsTo(subId, {
        onEvent: (_, content) => {
          ...
          if (content.kind === Kind.Metadata) {
            ...
            setReferencedNotes(subId, "users", (usrs) => ({
              ...usrs,
              [user.pubkey]: { ...user },
            }));
            return;
          ...
          if (content.kind === Kind.LiveEvent) {
            const streamData = {
              ...
              url: (content.tags?.find((t: string[]) => t[0] === "streaming") ||
                [])[1],
              image: (content.tags?.find((t: string[]) => t[0] === "image") ||
                [])[1],
              status: (content.tags?.find((t: string[]) => t[0] === "status") ||
                [])[1],
              starts: parseInt(
                (content.tags?.find((t: string[]) => t[0] === "starts") || [
                  "",
                  "0",
                ])[1]
              ),
              summary: (content.tags?.find(
                (t: string[]) => t[0] === "summary"
              ) || [])[1],
              title: (content.tags?.find((t: string[]) => t[0] === "title") ||
                [])[1],
              client: (content.tags?.find((t: string[]) => t[0] === "client") ||
                [])[1],
              currentParticipants: parseInt(
                (content.tags?.find(
                  (t: string[]) => t[0] === "current_participants"
                ) || ["", "0"])[1] || "0"
              ),
              pubkey: content.pubkey,
              hosts: (content.tags || [])
                .filter((t) => t[0] === "p" && t[3].toLowerCase() === "host")
                .map((t) => t[1]),
              participants: (content.tags || [])
                .filter((t) => t[0] === "p")
                .map((t) => t[1]),
            };
          ...
          if ([Kind.Text, Kind.Repost].includes(content.kind)) {
            ...
            setReferencedNotes(subId, "messages", (msgs) => [
              ...msgs,
              { ...message },
            ]);
          ...
          if (content.kind === Kind.NoteStats) {
            ...
            setReferencedNotes(subId, "postStats", (stats) => ({
              ...stats,
              [stat.event_id]: { ...stat },
            }));
            return;
          ...
          if (content.kind === Kind.Mentions) {
            ...
            setReferencedNotes(subId, "mentions", (mentions) => ({
              ...mentions,
              [mention.id]: { ...mention },
            }));
            return;
  ...
  const parseForReferece = async (value: string) => {
    const content = await replaceLinkPreviews(
      parseUserMentions(
        highlightHashtags(parseNote1(value, media?.actions.getMediaUrl))
      )
    );
  ...
  createEffect(() => {
    if (query().length === 0) {
      search?.actions.getRecomendedUsers(
        profile?.profileHistory.profiles || []
      );
      return;
    }
  ...
  const determineOrient = () => {
    ...
    const height = 326;
    return (coor.y || 0) + height < window.innerHeight + window.scrollY
      ? "down"
      : "up";
  };
  ...
  return (
    ...
    >
      ...
      <div class={styles.editorWrap} onClick={focusInput}>
        <div>
          ...
          <Show when={props.context}>
            <div class={styles.context}>{props.context}</div>
          </Show>
        ...
        >
          ...
          <div class={styles.uploader}>
            <UploaderBlossom
              ...
              onFail={() => {
                toast?.sendWarning(
                  intl.formatMessage(tUpload.fail, {
              ...
              onRefuse={(reason: string) => {
                if (reason === "file_too_big_100") {
                  toast?.sendWarning(
                    intl.formatMessage(tUpload.fileTooBigRegular)
                  );
                }
                if (reason === "file_too_big_1024") {
                  toast?.sendWarning(
                    intl.formatMessage(tUpload.fileTooBigPremium)
                  );
                }
                resetUpload();
              }}
      ...
      <Show when={isMentioning()}>
        <div class={styles.searchSuggestions} ref={mentionOptions}>
          <For each={search?.users}>
            {(user, index) => (
              <SearchOption
                ...
                statNumber={
                  profile?.profileHistory.stats[user.pubkey]?.followers_count ||
                  search?.scores[user.pubkey]
                }
      ...
      <Show
        when={
          isEmojiInput() &&
          emojiQuery().length > emojiSearchLimit &&
          emojiResults.length > 0
        }
      >
        <div class={styles.emojiSuggestions} ref={emojiOptions}>
          <For each={emojiResults}>
            {(emoji, index) => (
              <button
                id={`${instanceId}-${index()}`}
                class={`${styles.emojiOption} ${
                  highlightedEmoji() === index() ? styles.highlight : ""
                }`}
      ...
      <div class={styles.controls}>
        <div class={styles.editorOptions}>
          ...
          <div class={styles.editorOption}>
            ...
            >
              <div
                ref={emojiPicker}
                class={`emoji_icon ${styles.emojiIcon} ${
                  isPickingEmoji() ? styles.highlight : ""
                }`}
              ></div>
            </ButtonGhost>
        ...
        <div class={styles.editorDescision}>
          ...
          >
            {intl.formatMessage(tActions.notePostSubscribersNew)}
          </ButtonPrimary>
