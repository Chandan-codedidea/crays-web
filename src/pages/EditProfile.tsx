...
const EditProfile: Component = () => {
  ...
  const [uploadTarget, setUploadTarget] = createSignal<
    "picture" | "banner" | "none"
  >("none");
  ...
  const onUpload = (
    ...
    fileUpload: HTMLInputElement | undefined
  ) => {
    if (!fileUpload) {
      return;
    }
  ...
  const onSubmit = async (e: SubmitEvent) => {
    ...
    const userProfile: Record<string, any> = profile?.userProfile || {};
  ...
  return (
    <div class={styles.container}>
      ...
      <div id="central_header" class={styles.fullHeader}>
        <div
          id="profile_banner"
          class={`${styles.banner} ${flagBannerForWarning()}`}
        >
      ...
      <form onSubmit={onSubmit}>
        ...
        <div class={styles.inputWithPrefix}>
          <div class={styles.inputPrefix}>@</div>
          <input
            name="name"
            type="text"
            ref={nameInput}
        ...
        <input
          ...
          placeholder={intl.formatMessage(
            tSettings.profile.displayName.placeholder
          )}
          value={
        ...
        <div class={styles.inputLabel}>
          <label for="website">
            {intl.formatMessage(tSettings.profile.website.label)}
          </label>
        </div>
        <input
          name="website"
          type="text"
          placeholder={intl.formatMessage(
            tSettings.profile.website.placeholder
          )}
        ...
        <div class={styles.inputLabel}>
          <label for="about">
            {intl.formatMessage(tSettings.profile.about.label)}
          </label>
        </div>
        <textarea
          name="about"
        ...
        <div class={styles.inputLabel}>
          <label for="lud16">
            {intl.formatMessage(tSettings.profile.lud16.label)}
          </label>
        </div>
        <input
          name="lud16"
        ...
        <div class={styles.inputLabel}>
          <label for="nip05">
            {intl.formatMessage(tSettings.profile.nip05.label)}
          </label>
        </div>
        <input
          name="nip05"
        ...
        <div
          class={`${styles.moreInputs} ${
            isMoreVisible() ? styles.show : styles.hide
          }`}
        >
          <div class={styles.inputLabel}>
            <label for="picture">
              {intl.formatMessage(tSettings.profile.picture.label)}
            </label>
          </div>
          <input
            name="picture"
            type="text"
            placeholder={intl.formatMessage(
              tSettings.profile.picture.placeholder
            )}
            value={avatarPreview() || ""}
            onChange={(e: Event) => {
          ...
          <div class={styles.inputLabel}>
            <label for="banner">
              {intl.formatMessage(tSettings.profile.banner.label)}
            </label>
          </div>
          <input
            name="banner"
            type="text"
            placeholder={intl.formatMessage(
              tSettings.profile.banner.placeholder
            )}
            value={bannerPreview() || ""}
            onChange={(e: Event) => {
              const target = e.target as HTMLInputElement;
        ...
        <div class={styles.formSubmit}>
          <ButtonPrimary type="submit" disabled={!isNameValid()}>
            {intl.formatMessage(tActions.save)}
          </ButtonPrimary>
          <ButtonSecondary
            type="button"
            onClick={() =>
              navigate(app?.actions.profileLink(account?.publicKey) || "")
            }
          >
