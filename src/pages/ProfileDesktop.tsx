...
import {
  Tier,
  TierCost,
} from "../components/SubscribeToAuthorModal/SubscribeToAuthorModal";
...
const ProfileDesktop: Component = () => {
  ...
  const resolveHex = async (vanityName: string | undefined) => {
    ...
    if (params.npub?.startsWith("nprofile")) {
      hex = (nip19.decode(params.npub).data as ProfilePointer)
        .pubkey! as string;
    }
  ...
  const addToHome = () => {
    settings?.actions.addProfileHomeFeed(profileName(), profile?.profileKey);

    toaster?.sendSuccess(
      intl.formatMessage(tToast.addFeedToHomeSuccess, { name: profileName() })
    );
  };

  const removeFromHome = () => {
    settings?.actions.removeProfileHomeFeed(profile?.profileKey);

    toaster?.sendSuccess(
      intl.formatMessage(tToast.removeFeedFromHomeSuccess, {
        name: profileName(),
      })
    );
  };
  ...
  const unMuteProfile = () => {
    ...
    account.actions.removeFromMuteList(profile.profileKey, "user", () =>
  ...
  const doReportUser = () => {
    ...
    setContext(false);
    toaster?.sendSuccess(
      intl.formatMessage(tToast.noteAuthorReported, {
        name: userName(profile?.userProfile),
      })
    );
  };
  ...
  const copyProfileLink = () => {
    navigator.clipboard.writeText(
      `${window.location.host}${app?.actions.profileLink(getHex()) || ""}`
    );
    setContext(false);
  ...
  const customZapInfo: () => CustomZapInfo = () => ({
    ...
    onSuccess: (zapOption: ZapOption) => {
      ...
      toaster?.sendSuccess(
        intl.formatMessage(toastZapProfile, {
  ...
  const isVisibleLegend = () => {
    if (!profile?.profileKey || !app?.memberCohortInfo[profile.profileKey])
      return false;
    ...
    if (
      ...
      (app?.memberCohortInfo[profile.profileKey].expires_on || 0) >
  ...
  const commonFollowers = () => {
    if (!profile?.commonFollowers || profile.commonFollowers.length === 0)
      return [];
    ...
    let re = sorted.toSorted((a, b) => {
      const aIsLegend =
        app?.memberCohortInfo[a.pubkey]?.tier === "premium-legend";
      const bIsLegend =
        app?.memberCohortInfo[b.pubkey]?.tier === "premium-legend";
  ...
  return (
    <>
      <PageTitle
        title={intl.formatMessage(t.title, {
          name:
            profile?.userProfile?.displayName ||
            profile?.userProfile?.name ||
            truncateNpub(profileNpub()),
      ...
      <Show when={profile?.userProfile}>
        <div class="preload">
          <NoteImage
            ...
            mediaThumb={
              media?.actions.getMedia(banner() || "", "m") ||
              media?.actions.getMedia(banner() || "", "o") ||
              banner()
            }
      ...
      >
        <div id="central_header" class={styles.fullHeader}>
          <div
            id="profile_banner"
            class={`${styles.banner} ${flagBannerForWarning()} animated`}
          >
            <Show
          ...
          <div class={`${styles.userImage} animated`}>
            <div class={`styles.avatar`}>
              <div
                class={
                  isSmallScreen() ? styles.phoneAvatar : styles.desktopAvatar
                }
              >
                <Avatar
          ...
          <div class={`${styles.profileActions} animated`}>
            <div class={styles.contextArea}>
              <ButtonSecondary onClick={openContextMenu} shrink={true}>
            ...
            <ButtonSecondary
              onClick={() =>
                profile?.userProfile &&
                app?.actions.openProfileQr(profile?.userProfile)
              }
              shrink={true}
            >
            ...
            <Show
              when={
                !isCurrentUser() ||
                !account?.following.includes(profile?.profileKey || "")
              }
            >
                ...
                <div class={styles.smallAbout}>
                  <div class={styles.columnLeft}>
                    <div class={`${styles.basicInfoName} animated`}>
                      <div class={styles.text}>{profileName()}</div>
                      <Show
                        when={profile?.userProfile?.nip05 && verification()}
                      >
                        <div class={styles.vc}>
                          <VerificationCheck
                            user={profile?.userProfile}
                    ...
                    <Show when={profile?.userProfile?.website}>
                      <div class={`${styles.website} animated`}>
                        <a
                          href={rectifyUrl(profile?.userProfile?.website || "")}
                          target="_blank"
                        >
                  ...
                  <div class={styles.columnRight}>
                    <div class={`${styles.followings} animated`}>
                      <button
                        class={styles.stats}
                        onClick={() => setFollowsModal(() => "follows")}
                      >
                    ...
                    <Show when={profile?.userStats.time_joined}>
                      <div class={`${styles.joined} animated`}>
                        {intl.formatMessage(t.jointDate, {
                        ...
                        <div class={styles.avatars}>
                          <For each={commonFollowers()}>
                            {(follower, index) => (
                              <A
                                href={
                                  app?.actions.profileLink(follower.npub) || ""
                                }
                                class={styles.avatar}
      ...
      <Show when={profile?.profileKey && isProfileLoaded()}>
        <ProfileTabs setProfile={setProfile} profileKey={profile?.profileKey} />
      </Show>

      <ConfirmModal
        open={confirmReportUser()}
        description={intl.formatMessage(tActions.reportUserConfirm, {
          name: userName(profile?.userProfile),
        })}
        onConfirm={() => {
          doReportUser();
      ...
      <ConfirmModal
        ...
        description={intl.formatMessage(tActions.muteUserConfirm, {
          name: userName(profile?.userProfile),
        })}
        onConfirm={() => {
          doMuteUser();
