...
export type NostrSocialContent = {
...
export type NostrEOSE = [type: "EOSE", subkey: string];

export type NostrNotice = [type: "NOTICE", subkey: string, reason: string];
...
export type MegaFeedPage = {
  ...
  userFollowerIncrease: Record<
    string,
    { increase: number; ratio: number; count: number }
  >;
  wordCount: Record<string, number>;
...
export type RepostInfo = (
  page: FeedPage,
  message: NostrNoteContent
) => PrimalRepost;

export type MegaRepostInfo = (
  page: MegaFeedPage,
  message: NostrNoteContent
) => PrimalRepost;
